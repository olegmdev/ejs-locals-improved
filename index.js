...
var renderFile = module.exports = function(file, options, fn){
  ...
  if (!options.locals.blocks) {
    ...
    options.locals.setStylesheets = setStylesheets.bind(blocks.stylesheets);
    options.locals.setScripts = setScripts.bind(blocks.scripts);
  }
...
// bound to scripts Block in renderFile (multiple scripts)
function setScripts(paths) {
	for (var path in paths) {
		script.call(this, paths[path]['path'], paths[path]['media']);
	}	
	return this;
}

// bound to style sheets Block in renderFile (multiple style sheets)
function setStylesheets(paths) {
	for (var path in paths) {
		stylesheet.call(this, paths[path]['path'], paths[path]['media']);
	}	
	return this;
}
